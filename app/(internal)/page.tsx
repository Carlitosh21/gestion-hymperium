...
import { MessageSquare, Send, Link2, Users, TrendingUp, RefreshCw, BarChart3, AlertCircle, ThumbsDown, DollarSign } from 'lucide-react'
              ...
              <div className="bg-surface rounded-xl p-6 border border-border">
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={stats.dailyStats}>
                    ...
                    <XAxis dataKey="dia" stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)', fontSize: 12 }} />
                    <YAxis stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)' }} allowDecimals={false} domain={[0, 'auto']} />
                    <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-foreground)' }} />
                    <Legend />
                ...
                {stats.doloresRanking.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={stats.doloresRanking} layout="vertical" margin={{ left: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                      <XAxis type="number" stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)' }} />
                      <YAxis type="category" dataKey="dolor_normalizado" width={140} stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)', fontSize: 11 }} />
                      <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-foreground)' }} />
                ...
                {stats.objecionesRanking.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={stats.objecionesRanking} layout="vertical" margin={{ left: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                      <XAxis type="number" stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)' }} />
                      <YAxis type="category" dataKey="objecion_limpia" width={180} stroke="var(--color-muted)" tick={{ fill: 'var(--color-muted)', fontSize: 11 }} />
                      <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-foreground)' }} />
              ...
              <div className="bg-surface rounded-xl p-6 border border-border">
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={stats.costoDiario}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-foreground)' }} />
                    <Bar dataKey="costo_diario_usd" fill="var(--color-accent)" name="USD" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
